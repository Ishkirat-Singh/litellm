...
def install_statusline_script(script_path: Path = STATUSLINE_SCRIPT_PATH) -> str:
    ...
    Codex can run it without the litellm package being importable from their shell.
    """
    try:
        script_path.parent.mkdir(parents=True, exist_ok=True)
        script_path.write_bytes(Path(statusline_script.__file__).read_bytes())
    except OSError as e:
        raise ClaudeSettingsError(f"Could not install the status line script at {script_path}: {e}") from e
    return statusline_command(script_path)
...
def configure_claude_settings(
    ...
    script_path: Path = STATUSLINE_SCRIPT_PATH,
) -> None:
